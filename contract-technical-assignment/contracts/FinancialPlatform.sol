...
contract FinancialPlatform is AccessControl, ReentrancyGuard {
    ...
    /**
     * @dev Get recent transactions (last N transactions)
     */
    function getRecentTransactions(uint256 count) external view returns (uint256[] memory) {
        require(count > 0 && count <= _transactionIds, "Invalid count");
        
        uint256[] memory recentTransactions = new uint256[](count);
        uint256 startIndex = _transactionIds - count + 1;
        
        for (uint256 i = 0; i < count; i++) {
            recentTransactions[i] = startIndex + i;
        }
    ...
    function getAllTransactions() external view onlyAdmin returns (uint256[] memory) {
        uint256[] memory allTransactions = new uint256[](_transactionIds);
        
        for (uint256 i = 1; i <= _transactionIds; i++) {
            allTransactions[i - 1] = i;
        }
